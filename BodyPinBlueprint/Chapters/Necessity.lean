...
in, and its Lemma 6.1 is what turns the block-twist motion back into a flex of
the expanded graph.

The formalization takes 894 lines in the two modules that carry the direction,
...
impose two constraints rather than one, which is what the capacities $`3, 5, 6`
count. The endpoint maps are arbitrary functions, so nothing in this layer
depends on a graph library.
...
Part (a) is where the four private vertices earn their place in
{bpref "bodypin_expansion"}[the expansion]. A complete graph on four affinely
independent points is infinitesimally rigid, so its velocities come from a
twist; every other vertex of the body is joined to all four, and subtracting the
velocity that twist prescribes leaves a vector orthogonal to a spanning set.
...
{bpref "split_klein_form"}[the Split–Klein chapter], which is where the equation
$`q(X) = 0` is put to work: the whole sufficiency argument is a height estimate
for the ideal generated by these isotropy conditions.
...
This lemma is the reason a bundle of three or more pins between two blocks is
worth no more than a bundle of three. It is used again in
...
The pin capacities are what a single relative twist can be constrained by. One
pin is three linear conditions on a six-dimensional twist; two pins leave the
rotation about the line through them, so five; and no number of pins can remove
more than the six dimensions there are. Those three bounds are
...
The formalization proves the contrapositive of that argument, which is to say it
proves the statement directly. There is no partition violating the inequality
and no flex to construct. Twist rigidity at a pin placement makes the grouped
...
produce the placement it wants by hand, and this is where that costs something.

The construction is a one-parameter avoidance argument. A lower bound on the
