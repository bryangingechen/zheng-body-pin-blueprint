...
One thing that looks like an exception and is not. *The grounded inequality is
in §4* is an ordinary locative rather than this construction, and the checker
does not match it. A definition can take the first
...
- **Chapter prose names nothing a web reader cannot open.** The repository's
  own files — the deviations register, `notes/`, `scripts/`, `PLAN.md` — are
  for source comments and, sparingly, the correspondence chapter. A chapter
  states the fact a file records; the `deviation` tag already says a register
  entry exists. Modules and declarations of the pinned formalization are
  content, not repository plumbing, and stay.
